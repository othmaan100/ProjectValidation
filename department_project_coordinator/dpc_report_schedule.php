...
$stmt = $conn->prepare("SELECT * FROM report_schedules WHERE department_id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_override'])) {
    ...
    if (empty($student_id) || empty($ov_start) || empty($ov_end)) {
        ...
        try {
            $stmt = $conn->prepare("INSERT INTO student_report_overrides (student_id, submission_start, submission_end, is_active) 
...
if (isset($_GET['delete_override'])) {
    ...
    $stmt = $conn->prepare("DELETE FROM student_report_overrides WHERE id = ?");
    $stmt->execute([$ov_id]);
...
$overrides_stmt = $conn->prepare("
    ...
    FROM student_report_overrides o 
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_general'])) {
    ...
    if (empty($start) || empty($end)) {
        ...
        try {
            if ($schedule) {
                $stmt = $conn->prepare("UPDATE report_schedules SET submission_start = ?, submission_end = ?, is_active = ? WHERE department_id = ?");
                $stmt->execute([$start, $end, $is_active, $dept_id]);
            } else {
                $stmt = $conn->prepare("INSERT INTO report_schedules (department_id, submission_start, submission_end, is_active) VALUES (?, ?, ?, ?)");
...
<head>
    ...
    <title>Manage Final Report Schedule | DPC Portal</title>
...
<body>
    ...
    <div class="page-container">
        ...
        <div class="card">
            <div class="header">
                <h1><i class="fas fa-calendar-alt" style="color: var(--primary);"></i> General Final Report Schedule</h1>
                <p>Control the window for final report submissions for your department.</p>
            </div>
