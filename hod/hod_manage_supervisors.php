...
$stmt = $conn->prepare("SELECT s.*, sas.accepted_at FROM supervisors s LEFT JOIN supervisor_allocation_status sas ON s.id = sas.supervisor_id AND sas.session = :session WHERE $whereClause ORDER BY name ASC LIMIT $perPage OFFSET $offset");
                ...
                <tbody>
                    ...
                    <?php else: ?>
                        ...
                        <tr id="row-<?= $s['id'] ?>">
                            ...
                            <td>
                                <?php if ($s['accepted_at']): ?>
                                    <span class="status-badge" style="background: #1cc88a; color: white;"><i class="fas fa-check"></i> Accepted</span><br>
                                    <small style="color: #999; font-size: 10px;"><?= date('M j, Y', strtotime($s['accepted_at'])) ?></small>
                                <?php else: ?>
                                    <span class="status-badge" style="background: #f6c23e; color: white;"><i class="fas fa-clock"></i> Pending</span>
                                <?php endif; ?>
                            </td>
                            <td>
        ...
        document.querySelectorAll('form').forEach(f => {
            f.onsubmit = async (e) => {
                ...
                try {
                    ...
                    const res = await fetch('hod_manage_supervisors.php', { method: 'POST', body: fd });
