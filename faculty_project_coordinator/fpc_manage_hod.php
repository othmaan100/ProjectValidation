...
// Constant temporary password for all HODs
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax'])) {
    ...
    try {
        ...
        if (isset($_POST['action']) && $_POST['action'] === 'create') {
            ...
            // Insert new HOD
            $stmt = $conn->prepare("INSERT INTO users (username, name, email, password, role, department, faculty_id, session, is_active) VALUES (?, ?, ?, ?, 'hod', ?, ?, ?, 1)");
            $stmt->execute([$username, $name, $email, $hashedTempPassword, $departmentId, $faculty_id, $currentSession]);
            
            $response['success'] = true;
            $response['message'] = "HOD created successfully! Username: $username, Temporary Password: $constantTempPassword";
        ...
        elseif (isset($_POST['action']) && $_POST['action'] === 'update') {
            ...
            // Update HOD
            $stmt = $conn->prepare("UPDATE users SET username = ?, name = ?, email = ?, department = ? WHERE id = ? AND role = 'hod' AND faculty_id = ?");
        ...
        elseif (isset($_POST['action']) && $_POST['action'] === 'delete') {
            ...
            // Delete HOD
            $stmt = $conn->prepare("DELETE FROM users WHERE id = ? AND role = 'hod' AND faculty_id = ?");
        ...
        elseif (isset($_POST['action']) && $_POST['action'] === 'toggle_status') {
            ...
            // Toggle status
            $stmt = $conn->prepare("UPDATE users SET is_active = ? WHERE id = ? AND role = 'hod' AND faculty_id = ?");
        ...
        elseif (isset($_POST['action']) && $_POST['action'] === 'reset_password') {
            $id = intval($_POST['id']);
            // Reset password
            $stmt = $conn->prepare("UPDATE users SET password = ?, password_changed = 0 WHERE id = ? AND role = 'hod' AND faculty_id = ?");
        ...
        elseif (isset($_POST['action']) && $_POST['action'] === 'update_session') {
            ...
            // Deactivate HODs from previous sessions in this faculty
            $stmt = $conn->prepare("UPDATE users SET is_active = 0 WHERE role = 'hod' AND faculty_id = ? AND session != ?");
            $stmt->execute([$faculty_id, $newSession]);
            
            $currentSession = $newSession;
            
            $response['success'] = true;
            $response['message'] = "Session updated to $newSession. HODs from previous sessions have been deactivated.";
        }
...
// Fetch all HODs with pagination and search
$search = $_GET['search'] ?? '';
$page = isset($_GET['page']) ? max(1, intval($_GET['page'])) : 1;
$perPage = 10;
$offset = ($page - 1) * $perPage;

$whereClause = "u.role = 'hod' AND u.faculty_id = ?";
$params = [$faculty_id];
...
<head>
    ...
    <title>Manage HODs - Project Validation System</title>
...
<body>
    ...
    <div class="container">
        <div class="header">
            <div>
                <h1><i class="fas fa-users-cog"></i> Manage HODs</h1>
            ...
            <div class="header-actions">
                ...
                <button class="btn btn-primary" onclick="openCreateModal()">
                    <i class="fas fa-plus"></i> Add New HOD
                </button>
        ...
        <div class="card">
            ...
            <div class="table-container">
                ...
                <table>
                    ...
                    <tbody>
                        <?php foreach ($dpcs as $hod): ?>
                        <tr>
                            <td><strong><?php echo htmlspecialchars($hod['username']); ?></strong></td>
                            ...
                            <td><?php echo htmlspecialchars($hod['session'] ?? '-'); ?></td>
                            <td>
                                <label class="toggle-switch">
                                    <input type="checkbox" <?php echo $hod['is_active'] ? 'checked' : ''; ?> onchange="toggleStatus(<?php echo $hod['id']; ?>, this.checked)">
                                    <span class="slider"></span>
                                </label>
                                <span class="status-badge <?php echo $hod['is_active'] ? 'status-active' : 'status-inactive'; ?>">
                                    <?php echo $hod['is_active'] ? 'Active' : 'Inactive'; ?>
                                </span>
                            </td>
                            <td>
                                <div class="action-buttons">
                                    <button class="icon-btn btn-edit" onclick='openEditModal(<?php echo json_encode($hod); ?>)' title="Edit">
                                        <i class="fas fa-edit"></i>
                                    </button>
                                    <button class="icon-btn btn-reset" onclick="resetPassword(<?php echo $hod['id']; ?>)" title="Reset Password">
                                        <i class="fas fa-key"></i>
                                    </button>
                                    <button class="icon-btn btn-delete" onclick="deleteDPC(<?php echo $hod['id']; ?>, '<?php echo htmlspecialchars($hod['username']); ?>')" title="Delete">
                ...
                <div class="empty-state">
                    ...
                    <h3>No HODs Found</h3>
                    <p><?php echo !empty($search) ? 'No results match your search criteria.' : 'Click "Add New HOD" to create your first departmental project coordinator.'; ?></p>
                </div>
                <?php endif; ?>
            </div>
    ...
    <div id="dpcModal" class="modal">
        <div class="modal-content">
            <div class="modal-header">
                <h2 id="modalTitle">Add New HOD</h2>
                ...
                <form id="sessionForm">
                    ...
                    <div class="form-group">
                        ...
                        <small style="color: #666; display: block; margin-top: 8px;">
                            <i class="fas fa-info-circle"></i> This will deactivate all HODs from previous sessions.
                        </small>
                    </div>
    ...
    <script>
        ...
        function openCreateModal() {
            document.getElementById('modalTitle').textContent = 'Add New HOD';
        ...
        function openEditModal(hod) {
            document.getElementById('modalTitle').textContent = 'Edit HOD';
            document.getElementById('dpcId').value = hod.id;
            document.getElementById('username').value = hod.username;
            document.getElementById('name').value = hod.name || '';
            document.getElementById('email').value = hod.email || '';
            document.getElementById('department').value = hod.department;
        ...
        document.getElementById('dpcForm').addEventListener('submit', async function(e) {
            ...
            try {
                const response = await fetch('fpc_manage_hod.php', {
                    method: 'POST',
                    body: formData
                });
        ...
        document.getElementById('sessionForm').addEventListener('submit', async function(e) {
            ...
            if (!confirm('Are you sure you want to update the session? This will deactivate all HODs from previous sessions.')) {
                return;
            }
        ...
        async function deleteDPC(id, username) {
            if (!confirm(`Are you sure you want to delete HOD "${username}"? This action cannot be undone.`)) {
                return;
            }
        ...
        async function resetPassword(id) {
            if (!confirm('Are you sure you want to reset this HOD\'s password?')) {
                return;
            }
        ...
        function performSearch() {
            ...
            window.location.href = 'fpc_manage_hod.php?search=' + encodeURIComponent(search);
        }
        
        function clearSearch() {
            window.location.href = 'fpc_manage_hod.php';
        }
