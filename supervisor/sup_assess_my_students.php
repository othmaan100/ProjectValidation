...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_assessment'])) {
    ...
    try {
        ...
        ");
        $stmt->execute([$student_id, $supervisor_id, $score, $comments, $active_session]);
        $message = "Supervisor assessment saved successfully!";
...
<head>
    ...
    <title>Assess My Students | Project Pro</title>
    ...
    <style>
        ...
        .card { background: white; border-radius: 20px; box-shadow: 0 4px 15px rgba(0,0,0,0.05); margin-bottom: 30px; overflow: hidden; }
        .card-header { background: var(--primary); color: white; padding: 20px 30px; }
...
<body>
    ...
    <div class="container">
        <div class="header">
            <div>
                <h1>Supervisor Assessment</h1>
                <p style="color: #858796; margin: 5px 0 0 0;">Grade students under your direct supervision.</p>
            </div>
        ...
        <div class="card">
            <div class="card-header">
                <h2><i class="fas fa-user-graduate"></i> My Assigned Students (<?= $active_session ?>)</h2>
            </div>
            <?php if (empty($students)): ?>
                <div style="padding: 40px; text-align: center; color: #888;">You don't have any students assigned to you for supervision yet.</div>
            <?php else: ?>
                <table>
                    <thead>
                        <tr>
                    ...
                    <tbody>
                        <?php foreach ($students as $stu): ?>
                            <tr>
                                ...
                                <td>
                                    <div class="project-title"><?= htmlspecialchars($stu['project_title'] ?: 'No approved topic yet') ?></div>
                                </td>
                                <td>
                                    <?php if ($stu['score'] !== null): ?>
                                        <span class="score-badge score-yes"><i class="fas fa-check"></i> Graded</span>
                                    <?php else: ?>
                                        <span class="score-badge score-no"><i class="fas fa-times"></i> Pending</span>
                                    <?php endif; ?>
                                </td>
                                <td>
                                ...
                                <td style="text-align: right;">
                                    <button class="btn btn-assess" onclick="openAssessmentModal(
                                        ...
                                        <i class="fas fa-edit"></i> <?= $stu['score'] !== null ? 'Edit grade' : 'Grade Student' ?>
                                    </button>
                                </td>
                            </tr>
                        <?php endforeach; ?>
    ...
    <div id="assessmentModal" class="modal">
        <div class="modal-content">
            ...
            <form method="POST">
                ...
                <div class="form-group">
                    <label for="comments">Supervisor's Comments</label>
                    <textarea name="comments" id="modalComments" placeholder="Enter private progress notes or feedback..."></textarea>
                </div>
    ...
    <script>
        ...
        function openAssessmentModal(studentId, studentName, score, comments) {
